...
Everything shown here is either real, cited curated biology (the gene explorer,
interventions, references) or a clearly-labelled simulated worked example (the
example biomarker cohort). Nothing is fabricated evidence.
...
def render_clock():
    ...
    src = st.radio(
        "Data source", ["Simulated example cohort", "Upload my own CSV"], horizontal=True
    )
    df = None
    if src == "Simulated example cohort":
        df = svc.store.get_dataset("example_cohort_simulated")
        st.markdown(
            "<span class='gq-sim'>SIMULATED DATA</span> "
            "<span class='gq-note'>720 synthetic subjects — a transparent worked example, "
            "not real people.</span>",
            unsafe_allow_html=True,
        )
    else:
        st.caption(
...
def render_resilience():
    ...
    if src == "Simulated example cohort":
        ...
        st.markdown(
            "<span class='gq-sim'>SIMULATED DATA</span> "
            "<span class='gq-note'>720 synthetic subjects with an age-growing shared "
            "stress factor, so the early-warning signals are visible by construction.</span>",
            unsafe_allow_html=True,
        )
    else:
        st.caption(
...
with tab_about:
    ...
    with c1:
        st.markdown(
            ...
            "<div class='gq-note'>"
            "• <b>Gene aging biology</b> — directions, tissues, hallmarks, and analysis are curated "
            "from the published literature, each line carrying a real PubMed citation.<br><br>"
            "• <b>Curated database flags</b> — genuine memberships in GenAge, CellAge, LongevityMap "
            "and OpenGenes.<br><br>"
            "• <b>Interventions</b> — real lifespan-affecting compounds/diets with approximate "
            "reported rodent effects and primary citations (NIA ITP, DrugAge).<br><br>"
            "• <b>The clock</b> — the published Levine PhenoAge model with its real coefficients."
            "</div></div>",
            unsafe_allow_html=True,
        )
    with c2:
        st.markdown(
            ...
            "<div class='gq-note'>"
            "• <b>The example biomarker cohort</b> is <span class='gq-sim'>SIMULATED</span> — 720 "
            "synthetic subjects that exist only so the clock and resilience tools can be tried "
            "without uploading data. It is never presented as measurements of real people; "
            "upload-your-own is the first-class path.<br><br>"
